...
> **分享风险（T073-R06）**：`brain.db` 里的 `secrets` 表按设计保存明文 Key，因为它是本机单用户程序、需要把 Key 交给服务商使用。因此完整数据库文件**不是**可外发的诊断包。需要给别人看数据时，请用应用内的逻辑导出（设置页或 `GET /api/export`），不要直接发 `brain.db`。

## 1. 日常备份：逻辑导出

应用内导出，或直接调用本机接口（需带本机会话令牌）：
